...
class SimpleXMLReader extends \XMLReader
{
    ...
    const ANYNODE_UNPARSED = "ANYNODE_UNPARSED";
    ...
    public function parse() {
        ...
        $anyNodeCallback = NULL;
        if (isset ($this->callback[self::ANYNODE_UNPARSED]["*"]))
            $anyNodeCallback = $this->callback[self::ANYNODE_UNPARSED]["*"];

        while ($this->read()) {
            $walkIntoNode = true;
            if (isset($this->callback[$this->nodeType])) {
                if (isset($this->callback[$this->nodeType][$this->name])) {
                    $walkIntoNode = call_user_func($this->callback[$this->nodeType][$this->name], $this);
                } else {
                    $xpath = $this->currentXpath(false); // without node counter
                    if (isset($this->callback[$this->nodeType][$xpath])) {
                        $walkIntoNode = call_user_func($this->callback[$this->nodeType][$xpath], $this);
                    } else {
            ...
            */
            ...
            if ($walkIntoNode !== false && is_callable($anyNodeCallback)) {
                $anyNodeCallback($this);
            }
        }
    }
    /**
